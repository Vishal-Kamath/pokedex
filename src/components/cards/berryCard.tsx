...
const BerryCard: React.FC<{ id: string; name: string }> = ({ id, name }) => {
  return (
    <Link
      href={`/pokemon/${name}`}
      key={name}
    ...
    >
      <Image
        alt={name}
        src={`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/items/dream-world/${name}-berry.png`}
